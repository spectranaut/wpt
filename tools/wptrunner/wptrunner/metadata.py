...
def get_properties(properties_file=None, extra_properties=None, config=None, product=None):
    ...
    if properties_file is not None:
        ...
        try:
            with open(properties_file) as f:
                ...
                if "properties" not in data:
                    msg = f"Properties file missing 'properties' key"
                ...
                elif "dependent_properties" in data:
                    dependent_properties = data["dependent_properties"]
                    if not isinstance(dependent_properties, dict):
                        msg = "Properties file 'dependent_properties' value must be an object"
                    elif (not all(isinstance(dependent_properties[item], list) and
                                  all(isinstance(item_value, str)
                                      for item_value in dependent_properties[item])
                                  for item in dependent_properties)):
        ...
        properties, dependent_properties = products.load_product_update(config, product)

    if extra_properties is not None:
        properties.extend(extra_properties)

    return properties, dependent_properties
